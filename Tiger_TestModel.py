...
class CustomAgent(pomdp_py.Agent):
    ...
    def GetActionByPolicy(self,MyObs,HisObs):
        ...
        ob_tL_my_Obs = MyObs.histogram[TigerState('tiger-left')]
        ob_tR_my_Obs = MyObs.histogram[TigerState('tiger-right')]

        ob_tL_his_Obs = HisObs.histogram[TigerState('tiger-left')]
        ob_tR_his_Obs = HisObs.histogram[TigerState('tiger-right')]

        My_Obs_Final = [ob_tL_my_Obs, ob_tR_my_Obs]
        His_Obs_Final = [ob_tL_his_Obs, ob_tR_his_Obs]

        Q_target=self.model.predict(self.preprocess_state2(My_Obs_Final,His_Obs_Final))
        return self.ACTIONS[np.argmax(Q_target)]

    def GetActionByRandom(self,MyObs,HisObs):
    ...
    def choose_action2(self,my_Obs,his_Obs, epsilon):
        ...
        ob_tL_his_Obs = his_Obs.histogram[TigerState('tiger-left')]
        ob_tR_his_Obs = his_Obs.histogram[TigerState('tiger-right')]

        My_Obs_Final = [ob_tL_my_Obs, ob_tR_my_Obs]
        His_Obs_Final = [ob_tL_his_Obs, ob_tR_his_Obs]

        Processed_State=self.preprocess_state2(My_Obs_Final,His_Obs_Final)
    ...
    def Replay2(self,batch_size):
        ...
        for (my_obs, my_actionID, my_obs_new, my_JointReward, my_done),(his_obs, his_actionID, his_obs_new, his_JointReward, his_done) in zip(my_minibatch,his_minibatch):
            if my_JointReward!=his_JointReward:
                print("----- E R R O R --------")
            if my_done!=his_done:
                print("----- E R R O R --------")

            process_obs=self.preprocess_state2(my_obs,his_obs)
            Q_target = self.model.predict(process_obs)

            if my_done or his_done:
                Q_target[0][my_actionID] = my_JointReward
            else:
                process_obs_next = self.preprocess_state2(my_obs_new,his_obs_new)
    ...
    def Remeber2(self,MyHistory,HisHistory,JointReward,Done):
        ...
        self.MyHistory.append([My_OldObs, My_Action, My_NewObs, JointReward,Done])
...
if __name__ == '__main__':
    ...
    agent_1.LoadPolicy('Tiger_Model/Agent_1_Model_Agent_1NN_Agent_2ActionNN')
    ...
    agent_2 = CustomAgent(init_belief,
                       ...
                       ObservationModel(),
                       RewardModel(),'agent_2')
    agent_2.LoadPolicy('Tiger_Model/Agent_2_Model_Agent_1NN_Agent_2ActionNN')
    ...
    for Epizode in range(100):
        ...
        while (not Done):
            ...
            Agent_1_Action = agent_1.GetActionByPolicy(agent_1.cur_belief, agent_2.cur_belief)
            Agent_2_Action = agent_2.GetActionByPolicy(agent_2.cur_belief, agent_1.cur_belief)
            ...
            agent_1_history = [agent_1.OldBelief, Agent_1_Action, agent_1.cur_belief]
            agent_2_history = [agent_2.OldBelief, Agent_2_Action, agent_2.cur_belief]
            ...
            agent_1.Remeber(agent_1.OldOldBelief, agent_1.OldBelief, Agent_1_Action,
                            agent_1.cur_belief, join_reward, (Done1 or Done2))

            agent_2.Remeber(agent_2.OldOldBelief, agent_2.OldBelief, Agent_2_Action,
                            agent_2.cur_belief, join_reward, (Done1 or Done2))
