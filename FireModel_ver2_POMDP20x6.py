...
def GetLocStateNum(LocStateName):
    #print('LocStateName:'+LocStateName)
    if LocStateName=='h':
        return -1
    else:
...
class CustomAgent(pomdp_py.Agent):
    ...
    def __init__(self, init_belief, PolicyModel,TransitionModel,ObservationModel,RewardModel,AgentName,Model='',Epsilon=1.0,SequentiallyScouting={'Indx':[1,2,3],'Dir':'F'}):
        ...
        self.AGENT_STATES = ['r1', 'r2', 'r3', 'r4', 'r5', 'r6', 'r7', 'r8', 'r9', 'r10','r11', 'r12', 'r13', 'r14', 'r15', 'r16', 'r17', 'r18', 'r19', 'r20', 'h'];
        self.AGENT_ACTIONS = ['scout1', 'scout2', 'scout3', 'scout4', 'scout5', 'scout6', 'scout7', 'scout8', 'scout9', 'scout10','scout11', 'scout12', 'scout13', 'scout14', 'scout15', 'scout16', 'scout17', 'scout18', 'scout19', 'scout20', 'end'];
        self.F_STATES = ['u', 'f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'f8', 'f9', 'f10', 'f11', 'f12', 'f13', 'f14', 'f15', 'f16', 'f17', 'f18', 'f19', 'f20']
        self.SequentiallyScoutingIndx=SequentiallyScouting['Indx']
        self.SequentiallyScoutingDirection=SequentiallyScouting['Dir']
        ...
        if Model!='':
            ...
            #binary_input = Input(shape=(len(binary_data[0]),), name='binary_input')
            #belief_input = Input(shape=(len(belief_data[0]),), name='belief_input')
            ...
            self.model.compile(loss='mean_squared_error', optimizer=Adam(learning_rate=self.alpha))
    ...
    def GetBelief(self,Bt):
        init_belief = pomdp_py.Histogram({
            ...
            AgentState("f11"): Bt[11],
            AgentState("f12"): Bt[12],
            AgentState("f13"): Bt[13],
            AgentState("f14"): Bt[14],
            AgentState("f15"): Bt[15],
            AgentState("f16"): Bt[16],
            AgentState("f17"): Bt[17],
            AgentState("f18"): Bt[18],
            AgentState("f19"): Bt[19],
            AgentState("f20"): Bt[20]
        })
        return init_belief
    ...
    def StateReward(self,agent_state,AgentAction):
        ...
        if AgentAction.name!='end' and GetLocStateNum(agent_state)==GetActionNum(AgentAction.name):
            R=-500
        else:
            R=0
        return R

    def SequentialReward(self,agent_state,AgentAction):
        if(AgentAction=='end'):
            return 0
        InitScout=self.SequentiallyScoutingIndx
        Direction=self.SequentiallyScoutingDirection
        RoomPlus1=GetLocStateNum(agent_state)+1
        if RoomPlus1 > (len(self.AGENT_STATES) - 1):
            RoomPlus1=RoomPlus1%(len(self.AGENT_STATES) - 1)

        RoomPlus2=GetLocStateNum(agent_state)+2
        if RoomPlus2 > (len(self.AGENT_STATES) - 1):
            RoomPlus2 = RoomPlus2 % (len(self.AGENT_STATES) - 1)

        RoomPlus3=GetLocStateNum(agent_state)+3
        if RoomPlus3 > (len(self.AGENT_STATES) - 1):
            RoomPlus3 = RoomPlus3 % (len(self.AGENT_STATES) - 1)

        RoomPlus4=GetLocStateNum(agent_state)+4
        if RoomPlus4 > (len(self.AGENT_STATES) - 1):
            RoomPlus4 = RoomPlus4 % (len(self.AGENT_STATES) - 1)

        RoomMinus1 = GetLocStateNum(agent_state) - 1
        if RoomMinus1<1:
            RoomMinus1=(len(self.AGENT_STATES) - 1)+RoomMinus1

        RoomMinus2 = GetLocStateNum(agent_state) - 2
        if RoomMinus2<1:
            RoomMinus2=(len(self.AGENT_STATES) - 1)+RoomMinus2

        RoomMinus3 = GetLocStateNum(agent_state) - 3
        if RoomMinus3<1:
            RoomMinus3=(len(self.AGENT_STATES) - 1)+RoomMinus3

        RoomMinus4 = GetLocStateNum(agent_state) - 4
        if RoomMinus4<1:
            RoomMinus4=(len(self.AGENT_STATES) - 1)+RoomMinus4

        ActionNum=GetActionNum(AgentAction)

        if Direction=='F':

            if (agent_state=='h' and ActionNum==InitScout[0]) or (ActionNum==RoomPlus1):
                return 6
            elif (agent_state=='h' and ActionNum==InitScout[1]) or (ActionNum==RoomPlus2):
                return 3
            elif (agent_state=='h' and ActionNum==InitScout[2]) or (ActionNum==RoomPlus3):
                return 2
            elif (agent_state=='h' and ActionNum==InitScout[3]) or (ActionNum==RoomPlus4):
                return 1
            else:
                return 0
        else:
            if (agent_state=='h' and ActionNum==InitScout[0]) or (ActionNum==RoomMinus1):
                return 6
            elif (agent_state=='h' and ActionNum==InitScout[1]) or (ActionNum==RoomMinus2):
                return 3
            elif (agent_state=='h' and ActionNum==InitScout[2]) or (ActionNum==RoomMinus3):
                return 2
            elif (agent_state=='h' and ActionNum==InitScout[3]) or (ActionNum==RoomMinus4):
                return 1
            else:
                return 0
    ...
    def SequentialScountingReward(self,agent_state,AgentAction):
        ...
        R=self.SequentialReward(agent_state,AgentAction.name)
        R=R*10
        if(R==0):
            R=-10
        return R
    ...
    def GetAgentTotalReward(self,agent_state,agent_belief,AgentAction):
        ...
        print('                 STATE:' + str(agent_state) +'   -IS_UNDEFINED:'+str(IsUndefined)+ '     -ACTION:' + str(AgentAction) + '  -RW_secv:' + str(Scounting_Reward) + '  -RW_b:' + str(Belief_Reward)+ '  -RW_stat:' + str(State_Reward)  )
    ...
    def BeliefReward(self,Belief_Val,AgentAction):
        ...
        if AgentAction.name!='end':
            ActionID=int(GetActionNum(AgentAction.name))
...
#FF=ShareDate2(Agents[0].StatesHistory[-5],Agents[0].BeliefHistory[-5])
    ...
    def ChangeAgentState(self,Action):
        if(Action.name=='end'):
            ...
            self.cur_agent_state=[x for x in self.AGENT_STATES if GetLocStateNum(x) == GetActionNum(Action.name) ][0]
        self.cur_agent_state_index=self.AGENT_STATES.index(self.cur_agent_state)
        self.cur_agent_state_vector=np.zeros(len(self.AGENT_STATES))
        self.cur_agent_state_vector[self.cur_agent_state_index]=1
...
class ObservationModel(pomdp_py.ObservationModel):
    ...
    def probability(self, observation, next_state, action):
        ...
        if GetActionNum(action.name)==GetFireStateNum(next_state.name):
...
#Environment_State (Real)   , Action
    def sample(self, next_state, action):
        ...
        if action.name == "end":
            sampleProbs = [0.25,0.25,0.25,0.25]
        elif GetActionNum(action.name)==GetFireStateNum(next_state.name):
...
class ObservationModelOld(pomdp_py.ObservationModel):
    ...
    def probability(self, observation, next_state, action):
        ...
        if GetActionNum(action.name)==GetLocStateNum(next_state.name):
            if observation.name in {'video'}:
                return 0.4
            if observation.name in {'thermal'}:
                return 0.4
...
#Environment_State (Real)   , Action
    def sample(self, next_state, action):
        ...
        if action.name == "end":
            sampleProbs = [0.25,0.25,0.25,0.25]
        elif GetActionNum(action.name)==GetLocStateNum(next_state.name):
...
class TransitionModel(pomdp_py.TransitionModel):
    ...
    def get_all_states(self):
        ...
        return [AgentState(s) for s in {'u', 'f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'f8', 'f9','f10', 'f11', 'f12', 'f13', 'f14', 'f15', 'f16', 'f17', 'f18', 'f19','f20'}]
...
class PolicyModel(pomdp_py.RolloutPolicy):
    ...
    ACTIONS = {AgentAction(s)
              for s in ["scout1", "scout2", "scout3", "scout4", "scout5", "scout6", "scout7", "scout8", "scout9", "scout10",'scout11', 'scout12', 'scout13', 'scout14', 'scout15', 'scout16', 'scout17', 'scout18', 'scout19', 'scout20',"end"]}
...
if __name__ == '__main__':
    ModelFolderName ='POMDP20X6_AMD/FireModel_NewRW_NewCons_ver10'
    ExpirenceFileName='newCon_ver10_ep4'
    AgentsExpirienceFolder='POMDP20x6_AMD/Experience'
    ...
    init_true_state = random.choice([AgentState(s) for s in {'f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'f8', 'f9','f10','f11', 'f12', 'f13', 'f14', 'f15', 'f16', 'f17', 'f18', 'f19','f20'}])
    ...
    Models = [
              'POMDP20x6_AMD/FireModel_NewRW_NewCons_ver10/agent1_NN_model_verFirstTrain_Epizode_4_6300',
              'POMDP20x6_AMD/FireModel_NewRW_NewCons_ver10/agent2_NN_model_verFirstTrain_Epizode_4_6300',
              'POMDP20x6_AMD/FireModel_NewRW_NewCons_ver10/agent3_NN_model_verFirstTrain_Epizode_4_6300',
              'POMDP20x6_AMD/FireModel_NewRW_NewCons_ver10/agent4_NN_model_verFirstTrain_Epizode_4_6300',
              'POMDP20x6_AMD/FireModel_NewRW_NewCons_ver10/agent5_NN_model_verFirstTrain_Epizode_4_6300',
              'POMDP20x6_AMD/FireModel_NewRW_NewCons_ver10/agent6_NN_model_verFirstTrain_Epizode_4_6300',
              ]
    ...
    agent_1 = CustomAgent(init_belief,
                          ...
                          RewardModel(), 'agent1',Models[0],Epsilon,{'Indx':[1,2,3,4],'Dir':'F'})

    agent_2 = CustomAgent(init_belief,
                          PolicyModel(),
                          TransitionModel(),
                          ObservationModel(obs_noise),
                          RewardModel(), 'agent2',Models[1],Epsilon,{'Indx':[10,9,8,7],'Dir':'B'})

    agent_3 = CustomAgent(init_belief,
                          PolicyModel(),
                          TransitionModel(),
                          ObservationModel(obs_noise),
                          RewardModel(), 'agent3',Models[2],Epsilon,{'Indx':[4,5,6,7],'Dir':'F'})

    agent_4 = CustomAgent(init_belief,
                          PolicyModel(),
                          TransitionModel(),
                          ObservationModel(obs_noise),
                          RewardModel(), 'agent4',Models[2],Epsilon,{'Indx':[11,12,13,14],'Dir':'F'})

    agent_5 = CustomAgent(init_belief,
                          PolicyModel(),
                          TransitionModel(),
                          ObservationModel(obs_noise),
                          RewardModel(), 'agent5',Models[2],Epsilon,{'Indx':[20,19,18,17],'Dir':'B'})

    agent_6 = CustomAgent(init_belief,
                          PolicyModel(),
                          TransitionModel(),
                          ObservationModel(obs_noise),
                          RewardModel(), 'agent6',Models[2],Epsilon,{'Indx':[14,15,16,17],'Dir':'F'})

    MaxIteration=50
    Agents=[agent_1, agent_2, agent_3,agent_4,agent_5,agent_6]
    [agent.LoadLocalHistory(AgentsExpirienceFolder+'/LH_'+agent.AgentName+'_'+ExpirenceFileName) for agent in Agents]
    #Agents=[agent_1]
    NumberOfEpisodes=6305
    NumberOfEpisodesEnd=15500
    running_reward=[0,0,0,0,0,0,0]
    Rewards=[]
    ...
    for Epizode in range(NumberOfEpisodes,NumberOfEpisodesEnd):
        ...
        while (not DDD):
            ...
            if (Agents[0].InMission==False and Agents[1].InMission==False and Agents[2].InMission==False and Agents[3].InMission==False and Agents[4].InMission==False and Agents[5].InMission==False) or i>MaxIteration:#Agents[0].InMission==False or MaxIteration==i: #
                DDD=True
        ...
        if Epizode%30==0:
            ...
            [agent.Replay(batch_size=512,epochs=4024) for agent in Agents]
        ...
        if Epizode%300==0:
            ...
            [agent.model.save(ModelFolderName+'/'+agent.AgentName+'_NN_model_verFirstTrain_Epizode_4_'+str(Epizode)) for agent in Agents]

    [agent.model.save(ModelFolderName + '/' + agent.AgentName + '_NN_model_verFirstTrain_4_Final') for agent in Agents]
